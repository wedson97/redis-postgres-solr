...
class Cbo_ocupacao(Resource):
    ...
    def get(self):
        ...
        if cached_data:
            ...
            logger.info(f"Tempo de consulta no Redis: {((termico_requisicao_redis - comeco_requisicaoredis) * 1000):.2f} ms")
        ...
        logger.info(f"Tempo de consulta ao banco de dados: {((termico_requisicao_db - comeco_requisicao_db) * 1000):.2f} ms")
...
class Cbo_ocupacao_solr(Resource):
    ...
    def get(self, busca):
        ...
        solr = pysolr.Solr(f"http://host.docker.internal:8983/solr/cbo_ocupacao/select?q={consulta}&wt=json", timeout=10)

        inicio_solr = time.time()
        try:
            resultados = solr.search(consulta)
        except Exception as e:
            logger.error(f"Erro ao consultar Solr: {str(e)}")
        ...
        if resultados:
            ...
            logger.info(f"Tempo de consulta no Solr: {((fim_solr - inicio_solr) * 1000):.2f} ms")
            return dados_resultado, 200

        logger.warning("Nenhum dado encontrado no Solr.")
